...
test_case="OSF"
description="Test osf performance if overflow"
...
tc_1() {
  ...
  output=$(ping -W2 -c5 -s32 "${server_ipaddress}" | tail -1| awk '{print $4}' | cut -d '/' -f 2)
...
tc_2() {
  ...
  output=$(ping -W2 -c5 -s1412 "${server_ipaddress}" | tail -1| awk '{print $4}' | cut -d '/' -f 2)
  if [ "$?" -ne 0 ] || [ -z "$output" ]; then
    echo -n "ping6 error or timeout !"
    result=$FAIL
    echo " result= FAIL"
...
tc_3() {
  ...
  output=$(timeout --preserve-status 8s iperf3 -6 -u -c"${server_ipaddress}" --forceflush -t5 -l188 -b1M | awk '/receiver/ {print $7}')
  if [ "$?" -ne 0 ] || [ -z "$output" ]; then
...
tc_4() {
  ...
  output=$(timeout --preserve-status 8s iperf3 -6 -u -c"${server_ipaddress}" --forceflush -t5 -l1310 -b1M | awk '/receiver/ {print $7}')
  if [ "$?" -ne 0 ] || [ -z "$output" ]; then
    echo -n "iperf3 error or timeout !"
    result=$FAIL
    echo " result= FAIL"
...
tc_5() {
  ...
  output=$(timeout --preserve-status 8s iperf3 -6 -c"${server_ipaddress}" --forceflush -t5 -l188 -b1M | awk '/receiver/ {print $7}')
  if [ "$?" -ne 0 ] || [ -z "$output" ]; then
    echo -n "iperf3 error or timeout !"
    result=$FAIL
    echo " result= FAIL"
...
tc_6() {
  echo -n "TC 5: TCP iperf3 to ${server_ipaddress} chunk 2048 bytes, "
  output=$(timeout --preserve-status 8s iperf3 -6 -c"${server_ipaddress}" --forceflush -t5 -l2048 -b1M | awk '/receiver/ {print $7}')
  if [ "$?" -ne 0 ] || [ -z "$output" ]; then
    echo -n "iperf3 error or timeout !"
    result=$FAIL
    echo " result= FAIL"
