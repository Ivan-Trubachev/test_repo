...
test_case="OSF"
description="Test osf send mcast UDP message"
...
#######################################
tc_1() {
  echo -n "TC 1: Send message to ${OSF_MCAST_IPV6_ADDR}, port ${OSF_MCAST_IPV6_PORT}, interface ${OSF_INTERFACE} ..."
  output=$(timeout --preserve-status 3s python3 ${tests_dir}/sendmcastudp6.py ${OSF_MCAST_IPV6_ADDR} ${OSF_MCAST_IPV6_PORT} ${OSF_INTERFACE} 2>&1)
  echo " result= PASS"
